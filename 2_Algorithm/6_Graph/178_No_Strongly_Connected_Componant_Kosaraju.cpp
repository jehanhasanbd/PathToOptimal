...
void dfs_vanilla(ADJ_LIST &adjList, BOOL_MAP &visited, int source) {
    visited[source] = true;

    for (auto neighbor: adjList[source]) {
        if (!visited[neighbor]) {
            dfs_vanilla(adjList,visited,neighbor);
        }
    }
}
...
int strongly_Connected_Componant_Kosaraju(ADJ_LIST &adjList) {
    BOOL_MAP visited;
    stack<int> st;
    ...
    while (!st.empty()) {
        ...
        if (!visited[topNode]) {
            no_of_components++;
            dfs_vanilla(reverseAdjList,visited,topNode);
        }
    }
...
int main() {
    ...
    cout<< strongly_Connected_Componant_Kosaraju(g.adjList);
}